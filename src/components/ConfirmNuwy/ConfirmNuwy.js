...
function MyVerticallyCenteredModal(props) {
    ...
    return (<>
        ...
        <p className="text-modal">
          Al continuar estás aceptando todos nuestros términos y
           condiciones
          </p>
        </div>
        <button className="btn btn-modal">
<a href="/#2" className="link-modal-terminos" > Terminos & Condiciones</a>
<img src="/svg/rowRigth.svg" alt="row" className="icon-modal" />
        </button>
          </Modal.Body>
          <Modal.Footer className="d-flex flex-nowrap justify-content-center">
          <button type="button" className="btn btn-nuwy-modal " >
          Aceptar y continuar
        </button>
          <button className="btn-cancel " onClick={props.onHide}>Cancelar</button>
          </Modal.Footer>
          </Modal>
 </>
    );
...
function ConfirmNuwy() {
    ...
    return (
      <>
      <button type="button" className="btn btn-send-nuwy mt-2" onClick={() => setModalShow(true)}>
          Enviar Dinero
        </button>
